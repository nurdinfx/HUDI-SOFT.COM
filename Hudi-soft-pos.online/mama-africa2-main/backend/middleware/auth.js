...
export const auth = async (req, res, next) => {
  try {
    ...
    if (token.startsWith('demo-')) {
      ...
      const demoBranch = (licenseKey && licenseKey.length > 5) 
        ? await Branch.findOneAndUpdate(
            { branchCode: licenseKey.substring(0, 10).toUpperCase() },
            { $setOnInsert: { name: 'Demo ' + licenseKey.substring(0, 4), address: 'Online Node', isActive: true } },
            { upsert: true, new: true }
          )
        : await getOrCreateDemoBranch();
    ...
    try {
      ...
      let user = await User.findById(decoded.id).populate('branch');
      if (!user) {
        return res.status(401).json({ success: false, message: 'User not found' });
      }

      // MULTI-TENANCY HOOK: Ensure user is on the correct license branch
      if (licenseKey && licenseKey.length > 10) {
        let licenseBranch = await Branch.findOne({ branchCode: licenseKey.substring(0, 10).toUpperCase() });
        
        if (!licenseBranch) {
          // Create a dedicated branch for this license if it doesn't exist
          licenseBranch = await Branch.create({
            name: user.companyName || 'My Business',
            branchCode: licenseKey.substring(0, 10).toUpperCase(),
            address: 'Online POS Terminal',
            email: user.email,
            isActive: true
          });
        }

        // If user's current branch is different (e.g. they switched licenses or were on MAIN), update them
        if (!user.branch || user.branch.branchCode !== licenseBranch.branchCode) {
          user.branch = licenseBranch._id;
          await user.save();
          user = await User.findById(user._id).populate('branch');
        }
      }
