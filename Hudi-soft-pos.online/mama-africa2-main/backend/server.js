...
// Simplified CORS Configuration - ALLOW ALL FOR MOBILE CONNECTIVITY
app.use(cors({
  origin: true, // Allow all origins
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'Origin', 'X-License-Key']
}));
...
// API Endpoints
app.post('/api/v1/upload', auth, upload.single('image'), async (req, res) => {
  try {
    if (!req.file) {
...
app.use(['/api/v1/orders', '/api/v1/products', '/api/v1/customers', '/api/v1/tables', '/api/v1/expenses', '/api/v1/dashboard', '/api/v1/users', '/api/v1/settings', '/api/v1/purchases', '/api/v1/purchase-orders', '/api/v1/suppliers', '/api/v1/finance', '/api/v1/reports', '/api/v1/inventory', '/api/v1/employees', '/api/v1/attendance'], licenseCheck);
...
console.log(`[Static] Looking for dist at: ${distPath}`);
console.log(`[Static] dist exists: ${fs.existsSync(distPath)}`);

if (fs.existsSync(distPath)) {
  // Serve all built frontend assets
  app.use(express.static(distPath));
  // For ALL non-API routes, return index.html so React Router handles them
  app.get('*', (req, res, next) => {
    if (req.url.startsWith('/api/')) return next();
    res.sendFile(path.join(distPath, 'index.html'));
  });
} else {
  // dist folder missing — frontend was not built
  app.get('*', (req, res, next) => {
    if (req.url.startsWith('/api/')) return next();
    res.status(503).send(`
      <html><body style="font-family:sans-serif;padding:40px">
        <h2>⚠️ Frontend not built yet</h2>
        <p>The React frontend has not been built. Please check the Render build logs.</p>
        <p>Expected dist at: <code>${distPath}</code></p>
...
io.on('connection', (socket) => {
  ...
  socket.on('join-waiter', (branchId) => {
    socket.join(`waiter-${branchId}`);
    console.log(`👨‍🍳 Socket ${socket.id} joined waiter room for branch ${branchId}`);
  });

  // Staff creates / updates an order → broadcast everywhere
  socket.on('create-order', (orderData) => {
    ...
    socket.to(`waiter-${orderData.branch}`).emit('new-order', orderData);
  });

  // Kitchen / POS changes order status → broadcast to customer tracking + waiter
  socket.on('update-order-status', (data) => {
    const { orderId, status, branch } = data;
    io.to(`branch-${branch}`).emit('order-status-update', { orderId, status });
    io.to(`waiter-${branch}`).emit('order-status-update', { orderId, status });
    io.to(`kitchen-${branch}`).emit('order-status-update', { orderId, status });
  });
