...
export const rejectPurchaseOrder = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: formatPurchaseOrder(po), message: 'Purchase order rejected' });
  } catch (error) {
    console.error('Reject purchase order error:', error);
    res.status(500).json({ success: false, message: error.message || 'Failed to reject purchase order' });
  }
};

// Delete purchase order - MongoDB
export const deletePurchaseOrder = async (req, res) => {
  try {
    const { id } = req.params;
    const branchId = req.user.branch._id || req.user.branch.id;

    const po = await PurchaseOrder.findOneAndDelete({ _id: id, branch: branchId });
