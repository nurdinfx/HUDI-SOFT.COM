...
export const addLedgerTransaction = async (req, res) => {
  ...
  try {
    ...
    // Determine new balance depending on proper account type sync
    const txAmount = parseFloat(amount);
    let currentBalance = 0;

    // Sync with Customer model fields precisely based on their account setup
    if (customer.accountType === 'debit') {
      if (type === 'debit' || type === 'sale') { // Customer owes us (reduce deposit)
        customer.balance -= txAmount;
      } else { // Customer payment (add to deposit)
        customer.balance += txAmount;
      }
      currentBalance = -customer.balance;
    } else if (customer.accountType === 'credit') {
      if (type === 'debit' || type === 'sale') { // Customer used more credit / owes us
        customer.creditUsed += txAmount;
      } else { // Customer paid off their credit limit
        customer.creditUsed = Math.max(0, customer.creditUsed - txAmount);
      }
      currentBalance = customer.creditUsed;
    } else {
      // Standard or unmigrated customer logic uses a generic running balance
      const lastTx = await CustomerLedger.findOne({ customer: customerId })
        .sort({ date: -1, _id: -1 })
        .session(session);
      
      currentBalance = lastTx ? lastTx.balance : 0;
      if (type === 'debit' || type === 'sale') {
        currentBalance += txAmount;
      } else {
        currentBalance -= txAmount;
      }
      // For standard customers, we also store it in balance to keep it visible
    ...
    res.status(500).json({ success: false, message: 'Failed to add transaction', debug: error.message, stack: error.stack });
  }
};
...
export const getCustomerSummary = async (req, res) => {
  try {
    ...
    // Calculate dynamic totals for debit and credit
    const ledgerStats = await CustomerLedger.aggregate([
      { $match: { customer: new mongoose.Types.ObjectId(id) } },
      { $group: {
        _id: null,
        totalDebit: { $sum: { $cond: [{ $in: ["$transactionType", ["credit", "payment"]] }, "$amount", 0] } },
        totalCredit: { $sum: { $cond: [{ $in: ["$transactionType", ["debit", "sale"]] }, "$amount", 0] } }
      }}
    ]);
    const stats = ledgerStats.length > 0 ? ledgerStats[0] : { totalDebit: 0, totalCredit: 0 };
