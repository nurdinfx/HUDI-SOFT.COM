...
export const addLedgerTransaction = async (req, res) => {
  ...
  try {
    ...
    if (customer.accountType === 'debit') {
      if (type === 'debit') { // 'Debit': Customer owes us (reduce deposit)
        customer.balance -= txAmount;
      } else { // 'Credit': Customer payment (add to deposit)
        customer.balance += txAmount;
      }
      currentBalance = customer.balance;
    } else if (customer.accountType === 'credit') {
      if (type === 'debit') { // 'Debit': Customer used more credit / owes us
        customer.creditUsed += txAmount;
      } else { // 'Credit': Customer paid off their credit limit
