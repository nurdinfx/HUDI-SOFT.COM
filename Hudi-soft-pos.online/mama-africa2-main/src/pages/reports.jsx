...
const Reports = () => {
  ...
  const [selectedMobileMethod, setSelectedMobileMethod] = useState('all');
  ...
  useEffect(() => {
    if (activeReport) {
      ...
      const interval = setInterval(() => {
        loadReportData(activeReport, true);
      }, 15000);
  ...
  const loadReportData = async (reportId, silent = false) => {
    try {
  ...
  const loadMobilePaymentReport = async (startDate, endDate) => {
    ...
    const fetchStartDate = new Date(Math.min(yearStart.getTime(), startDate.getTime()));
    ...
    let allOrders = realApi.extractData(response) || [];
    ...
    const getStatsForMethod = (methodKey) => {
      const filtered = allOrders.filter(o => 
        methodKey === 'all' 
          ? true 
          : (o.paymentMethod || '').toLowerCase() === methodKey.toLowerCase()
      );
      ...
      filtered.forEach(o => {
        ...
        if (oDate >= currentYearStart) {
          stats.year.amount += amount;
          stats.year.count++;
        }
      });
    ...
    const statsBreakdown = {
      all: getStatsForMethod('all'),
      zaad: getStatsForMethod('zaad'),
      sahal: getStatsForMethod('sahal'),
      edahab: getStatsForMethod('edahab'),
      mycash: getStatsForMethod('mycash'),
      mobile: getStatsForMethod('mobile')
    };

    // Filter current date range orders for the table view
    let filteredOrders = allOrders.filter(order => {
      const orderDate = new Date(order.createdAt || order.orderDate);
      return orderDate >= startDate && orderDate <= endDate;
    });

    // Apply active filters (cashier, etc.)
    filteredOrders = applyOrderFilters(filteredOrders);
    ...
    return {
      ...
      statsBreakdown,
      total: filteredOrders.reduce((sum, o) => sum + (o.finalTotal || o.totalAmount || 0), 0),
      count: filteredOrders.length,
      vat: filteredOrders.reduce((sum, o) => sum + (o.taxAmount || 0), 0)
    };
  };
  ...
  const renderReportContent = () => {
    ...
    switch (reportData.type) {
      ...
      case 'mobile-payment':
        ...
        // Get stats breakdown for selected method
        const currentStats = reportData.statsBreakdown?.[selectedMobileMethod] || {
          today: { amount: 0, count: 0 },
          week: { amount: 0, count: 0 },
          month: { amount: 0, count: 0 },
          year: { amount: 0, count: 0 }
        ...
        return (
          <div className="space-y-6">
            {/* Mobile Method Filter and Temporal Summary Dashboard */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <h2 className="text-lg font-bold text-gray-800">Mobile Payment Dashboard</h2>
                  <p className="text-xs text-gray-500">Real-time stats breakdown by today, week, month, and year</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-gray-600">Select Service:</span>
                  <select
                    className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm font-medium bg-gray-50 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all cursor-pointer"
                    value={selectedMobileMethod}
                    onChange={(e) => setSelectedMobileMethod(e.target.value)}
                  >
                    <option value="all">📱 All Mobile Methods</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 rounded-xl border border-blue-100 shadow-sm transition-all hover:shadow-md">
                  <span className="text-[10px] uppercase font-bold text-blue-600 tracking-wider">Today's Income</span>
                  <p className="text-2xl font-black text-blue-900 mt-1">{formatCurrency(currentStats.today.amount, settings?.currency || 'USD')}</p>
                  <span className="text-xs text-blue-600/80 font-medium block mt-1">⚡ {currentStats.today.count} transactions</span>
                </div>

                <div className="bg-gradient-to-br from-green-50 to-green-100/50 p-4 rounded-xl border border-green-100 shadow-sm transition-all hover:shadow-md">
                  <span className="text-[10px] uppercase font-bold text-green-600 tracking-wider">This Week's Income</span>
                  <p className="text-2xl font-black text-green-900 mt-1">{formatCurrency(currentStats.week.amount, settings?.currency || 'USD')}</p>
                  <span className="text-xs text-green-600/80 font-medium block mt-1">📅 {currentStats.week.count} transactions</span>
                </div>

                <div className="bg-gradient-to-br from-purple-50 to-purple-100/50 p-4 rounded-xl border border-purple-100 shadow-sm transition-all hover:shadow-md">
                  <span className="text-[10px] uppercase font-bold text-purple-600 tracking-wider">This Month's Income</span>
                  <p className="text-2xl font-black text-purple-900 mt-1">{formatCurrency(currentStats.month.amount, settings?.currency || 'USD')}</p>
                  <span className="text-xs text-purple-600/80 font-medium block mt-1">📊 {currentStats.month.count} transactions</span>
                </div>

                <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 p-4 rounded-xl border border-amber-100 shadow-sm transition-all hover:shadow-md">
                  <span className="text-[10px] uppercase font-bold text-amber-600 tracking-wider">This Year's Income</span>
                  <p className="text-2xl font-black text-amber-900 mt-1">{formatCurrency(currentStats.year.amount, settings?.currency || 'USD')}</p>
                  <span className="text-xs text-amber-600/80 font-medium block mt-1">🏆 {currentStats.year.count} transactions</span>
                </div>
              </div>
            </div>

            {/* Breakdown Bars / Percentage Analysis */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-md border border-gray-100 space-y-4">
                <h3 className="text-base font-bold text-gray-800">Method Breakdown</h3>
                <p className="text-xs text-gray-500">Share of mobile payment volume (Active Filter Range)</p>
                <div className="space-y-4 pt-2">
                  {reportData.mobileGroups?.map((group, index) => {
                    const totalMobileVolume = reportData.total || 1;
                    const percent = ((group.amount / totalMobileVolume) * 100).toFixed(1);
                    return (
                      <div key={index} className="space-y-1">
                        <div className="flex justify-between text-xs font-semibold text-gray-700">
                          <span className="capitalize">{group.method}</span>
                          <span>{percent}% ({formatCurrency(group.amount, settings?.currency || 'USD')})</span>
                        </div>
                  ...
                  {(!reportData.mobileGroups || reportData.mobileGroups.length === 0) && (
                    <p className="text-xs text-gray-400 italic text-center py-6">No data available for this range</p>
                  )}
                </div>
              </div>
              ...
              <div className="lg:col-span-2 bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden flex flex-col">
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                  <h3 className="text-base font-bold text-gray-800">Filtered Transactions</h3>
                  <span className="bg-blue-50 text-blue-700 text-xs font-bold px-2.5 py-1 rounded-full">{reportData.orders?.length || 0} orders</span>
                </div>
                <div className="overflow-x-auto flex-1">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {(reportData.orders || []).slice(indexOfFirstItem, indexOfLastItem).map((order, index) => (
                        <tr key={order._id || index} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-900">
                            #{(order.orderNumber || '').split('-').pop() || order._id.substring(18)}
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-right text-green-600">
                            {formatCurrency(order.finalTotal || order.totalAmount || 0, settings?.currency || 'USD')}
                          </td>
                        </tr>
                      ))}
                      {(!reportData.orders || reportData.orders.length === 0) && (
                        <tr>
                          <td colSpan="5" className="px-6 py-8 text-center text-gray-400 italic">No transactions found</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                </div>
                {renderPagination(Math.ceil((reportData.orders?.length || 0) / itemsPerPage))}
              </div>
            </div>
          </div>
        );
