...
const ActivationPage = () => {
    ...
    useEffect(() => {
        const checkStatus = () => {
            ...
            const cleanKeyFromUrl = keyFromUrl ? keyFromUrl.trim().toUpperCase() : '';

            // Use a persistent device ID from localStorage
            ...
            // 1. If key is in URL, force activation check/entry for this key
            if (cleanKeyFromUrl) {
                setStatus('unactivated');
                setLicenseKey(cleanKeyFromUrl);
                
                // Clear old stored key if it's different to force new activation
                const storedKey = localStorage.getItem('pos_license_key');
                if (storedKey && storedKey.trim().toUpperCase() !== cleanKeyFromUrl) {
                    localStorage.removeItem('pos_license_key');
                }
            } else {
                // 2. Otherwise, check if already activated
                const storedKey = localStorage.getItem('pos_license_key');
                if (storedKey) {
                    setStatus('activated');
                } else {
    ...
    const handleActivate = async (e) => {
        ...
        try {
            ...
            if (response.data?.success || response.data?.valid) {
                ...
                localStorage.setItem('pos_license_data', JSON.stringify({
                    valid: true,
                    key: normalizedKey,
                    expiryDate: response.data.expiryDate,
                    activatedAt: response.data.activationDate || new Date().toISOString(),
                    lastCheckedAt: new Date().toISOString()
                }));
                setStatus('activated');
                setLicenseKey(normalizedKey);
                setMessage(response.data?.message || 'License activated successfully.');
            } else {
