  ...
  */
  ...
  return (
    <div className="page-content flex flex-col gap-6 h-full overflow-auto">
              ...
              <div className="flex items-center">
                ...
                {table.qrToken && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ml-2 ${
                    table.qrEnabled ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-red-100 text-red-850 border border-red-200'
                  }`}>
                    QR {table.qrEnabled ? 'Active' : 'Disabled'}
                  </span>
                )}
              </div>
            ...
            <div className="flex justify-between space-x-2">
              ...
              {table.qrToken && (
                <button
                  ...
                  className="bg-indigo-650 hover:bg-indigo-700 text-white py-2 px-3 rounded text-sm font-medium"
                  title="View / Print QR"
                >
                  QR
                </button>
      ...
      {/* QR Code Modal — High Quality SVG */}
      {qrModalTable && (() => {
        const qrSvgRef = React.createRef();
        const rawUrl = qrModalTable.qrUrl || `${window.location.origin}/order?table=${qrModalTable.qrToken}`;
        const qrValue = rawUrl.replace('/menu?table=', '/order?table=');
        const tableNum = qrModalTable.number || qrModalTable.tableNumber;

        const downloadPNG = () => {
          const svgEl = qrSvgRef.current && qrSvgRef.current.querySelector('svg');
          if (!svgEl) { alert('QR not found'); return; }
          const SIZE = 1024;
          ...
          img.onload = () => {
            ...
            a.download = `table-${tableNum}-qr-1024px.png`;
        ...
        const printCard = () => {
          ...
          svgClone.setAttribute('width', '300'); svgClone.setAttribute('height', '300');
          const svgHtml = svgClone.outerHTML;
          const printWindow = window.open('', '_blank', 'width=600,height=700');
          printWindow.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>QR - Table ${tableNum}</title><style>*{box-sizing:border-box;margin:0;padding:0}@page{size:100mm 130mm;margin:0}body{font-family:'Helvetica Neue',Arial,sans-serif;background:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;padding:8mm}.card{width:88mm;border:2.5px solid #0f172a;border-radius:6mm;padding:6mm 7mm;text-align:center;background:#fff}.brand{font-size:14pt;font-weight:900;color:#0f172a;margin-bottom:1.5mm}.tagline{font-size:7pt;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.12em;margin-bottom:4mm}.qr-wrap{display:inline-block;background:#f8fafc;border:1px solid #e2e8f0;border-radius:4mm;padding:4mm;margin-bottom:4mm}.qr-wrap svg{display:block;width:60mm!important;height:60mm!important}.table-badge{display:inline-block;background:#0f172a;color:#fff;font-size:20pt;font-weight:900;padding:2mm 6mm;border-radius:3mm}.instructions{font-size:6.5pt;color:#475569;margin-top:3mm;line-height:1.6}</style></head><body><div class="card"><div class="brand">MAMA AFRICA RESTAURANT</div><div class="tagline">Scan &amp; Order Instantly</div><div class="qr-wrap">${svgHtml}</div><div><div class="table-badge">TABLE ${tableNum}</div></div><div class="instructions">Point your smartphone camera at the QR code above.<br>Browse our menu and place your order - no app needed.</div></div><script>window.onload=function(){setTimeout(function(){window.print();setTimeout(function(){window.close();},800);},300);}<\/script></body></html>`);
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden border border-slate-200">
              <div className="bg-slate-900 text-white p-4 flex justify-between items-center">
                <h2 className="text-base font-black flex items-center gap-2">
                  📱 Table {tableNum} QR Code
                </h2>
                <button onClick={() => setQrModalTable(null)} className="text-slate-400 hover:text-white text-2xl leading-none">×</button>
              </div>

              <div className="p-6 flex flex-col items-center gap-4">
                <div
                  ref={qrSvgRef}
                  className="bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-inner"
                  style={{ lineHeight: 0 }}
                >
                  <QRCodeSVG
                    value={qrValue}
                ...
                <div className="w-full bg-slate-50 rounded-lg p-3 text-center">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Scan URL</p>
                ...
                <p className="text-xs text-slate-500 font-semibold text-center">
                  ✅ Level H error correction · SVG vector quality · 280px display
                </p>

                <div className="w-full flex gap-2">
                  <button
                    onClick={downloadPNG}
                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg text-xs font-black transition-colors"
                  >
                    ⬇ PNG (1024px)
                  </button>
                  <button
                    onClick={printCard}
                    className="flex-1 bg-slate-900 hover:bg-slate-800 text-white py-2.5 rounded-lg text-xs font-black transition-colors"
                  >
                    🖨 Print Ticket
                  </button>
                </div>
