...
const Kitchen = ({ isPosMode = false }) => {
  ...
  const playSound = (type) => {
    try {
      const audioMap = {
        'new-order': '/new-notification.mp3',
        'order-ready': '/order-ready.mp3'
      };
      const audio = new Audio(audioMap[type] || '/new-notification.mp3');
      audio.play().catch(e => console.log('Audio playback prevented by browser:', e.message));
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (socket && isConnected) {
      ...
      socket.on('qr-new-order', handleNewOrder);
    ...
    return () => {
      ...
      if (socket) {
        ...
        socket.off('qr-new-order');
        socket.off('order-status-updated');
        socket.off('order-completed');
      }
    };
  }, [socket, isConnected, soundEnabled, loadKitchenOrders, setOrders, user]); // Added dependencies
  ...
  */
  ...
  const pendingOrders = Array.isArray(orders) ? orders.filter(order => ['pending', 'accepted', 'confirmed', 'delayed'].includes(order.kitchenStatus || order.status)) : [];
      ...
      <div className="p-3 border-b border-slate-100 flex justify-between items-start bg-slate-50/50 rounded-t-lg">
        <div>
          <div className="flex items-center space-x-2 mb-1">
            ...
            {order.orderSource === 'qr' && <span className="px-2 py-0.5 bg-blue-600 text-white text-[10px] font-bold rounded flex items-center gap-0.5">📱 QR</span>}
          </div>
