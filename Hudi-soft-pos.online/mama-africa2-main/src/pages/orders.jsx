...
const Orders = () => {
  ...
  useEffect(() => {
    calculateSummary();
  }, [filteredOrders, restaurantSettings]);
  ...
  const processPayment = async (orderId, paymentData) => {
    try {
      ...
      if (response.success) {
        let updatedOrderForPrint = null;

        setOrders(prev =>
          prev.map(order => {
            if (order._id === orderId) {
              const matchedCustomer = paymentData.customerId ? customers.find(c => String(c._id) === String(paymentData.customerId)) : order.customer;
              updatedOrderForPrint = {
                ...order,
                paymentStatus: 'paid',
                status: 'completed',
                paymentMethod: paymentData.paymentMethod,
                customer: matchedCustomer || order.customer,
                customerName: matchedCustomer?.name || order.customerName || 'Walking Customer',
                paidAt: new Date().toISOString()
              };
              return updatedOrderForPrint;
            }
            return order;
          })
        );
        ...
        if (updatedOrderForPrint && await shouldAutoPrintAfterPayment(restaurantSettings?.printReceipt)) {
          printReceipt(updatedOrderForPrint);
        }
  ...
  const handleTableInvoice = () => {
    ...
    printToIframe(receiptContent, {
      ...
      paperWidth: restaurantSettings?.receiptSize || '80mm',
    });
  };
  ...
  const handlePayNow = (order) => {
    ...
    // Prefill selected payment customer
    const orderCustomerId = order.customer?._id || order.customer;
    const foundCustomer = customers.find(c => String(c._id) === String(orderCustomerId));
    setSelectedPaymentCustomer(foundCustomer || null);
  ...
  const handlePaymentSubmit = () => {
    if (paymentOrder && paymentAmount) {
      if (paymentMethod === 'credit' && !selectedPaymentCustomer) {
        alert('Please select a customer for credit (ledger) payment.');
        return;
      }
      processPayment(paymentOrder._id, {
        paymentMethod,
        amount: parseFloat(paymentAmount),
        customerId: selectedPaymentCustomer?._id || null
      });
    }
  };
    ...
    <div className="pos-fullscreen-container bg-[#f0f2f5] flex flex-col h-screen overflow-hidden text-sm font-sans">
      ...
      <div className="bg-[#e9ecef] border-b border-gray-300 p-2 shrink-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-12 gap-2 items-end">
          ...
          {/* Reset/Action */}
          <div className="col-span-1 lg:col-span-1">
            <button onClick={() => setFilters({ status: '', customer: '', table: '', servedBy: '', room: '', search: '', dateFrom: '', dateTo: '' })}
          ...
          <table className="w-full border-collapse">
            <thead className="bg-[#f8f9fa] border-b border-gray-300 text-[10px] md:text-xs text-gray-600 sticky top-0 z-10">
              <tr>
                ...
                <th className="p-2 text-left hidden xl:table-cell">Remarks</th>
              ...
              {currentItems.length === 0 ? (
                ...
                currentItems.map((order, idx) => (
                  <tr key={order._id || idx} className="hover:bg-blue-50 transition-colors">
                    ...
                    <td className="p-2 align-top text-[10px] md:text-xs text-gray-500 italic max-w-xs truncate hidden xl:table-cell">
                      {order.notes || '-'}
                    </td>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white focus:border-blue-500 outline-none"
                    value={paymentMethod}
                  ...
                  <div>
                    ...
                    <select
                      className={`w-full border rounded-lg px-3 py-2 bg-white outline-none focus:border-blue-500 ${
                        !selectedPaymentCustomer ? 'border-red-400 bg-red-50 text-red-700' : 'border-green-400 bg-green-50 text-green-800'
                      }`}
                      value={selectedPaymentCustomer ? selectedPaymentCustomer._id : ""}
                      onChange={(e) => {
                        const found = customers.find(c => c._id === e.target.value);
                        setSelectedPaymentCustomer(found || null);
                      }}
                    >
                    ...
                    {!selectedPaymentCustomer && (
                      <p className="mt-1 text-xs text-red-500 font-semibold animate-pulse">
                        ⚠ Please select a customer to charge to credit.
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    ...
                    disabled={paymentMethod === 'credit' && !selectedPaymentCustomer}
        ...
        <div className="p-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {order.notes && (
                <div className="col-span-2 bg-yellow-50/50 p-3 rounded border border-yellow-100">
                  <h3 className="text-xs font-semibold text-yellow-800 uppercase tracking-wider">Order Remarks / Notes</h3>
                  <p className="mt-1 text-sm text-gray-900 font-medium whitespace-pre-wrap">
                    {order.notes}
                  </p>
                </div>
              )}
