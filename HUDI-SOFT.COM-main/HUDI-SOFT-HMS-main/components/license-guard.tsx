...
import { licenseApi, setLicenseKey, normalizeLicenseKey } from '@/lib/api';
...
export function LicenseGuard({ children }: { children: React.ReactNode }) {
    ...
    const activateWithKey = useCallback(async (rawKey: string) => {
        ...
        try {
            ...
            setIsValid(false);
            const message =
                err instanceof Error ? err.message : 'Activation failed. Check your connection and try again.';
            setError(message);
        } finally {
            setActivating(false);
            setChecking(false);
    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            if (urlKey) {
                const normalized = normalizeLicenseKey(urlKey);
                setKey(normalized);
            ...
            setKey(normalizeLicenseKey(storedKey));
            try {
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    spellCheck={false}
                                    disabled={activating}
                                    className="w-full pl-14 pr-6 py-5 bg-slate-800/50 border border-white/5 rounded-2xl focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 transition-all outline-none font-mono text-white text-lg placeholder:opacity-20 uppercase tracking-widest disabled:opacity-60"
                                />
                            </div>
                        </div>
                        ...
                        >
                            {activating ? (
                                <>
                                    ...
                                    <span>Connecting to license server…</span>
                                </>
                            ) : (
                                <>
