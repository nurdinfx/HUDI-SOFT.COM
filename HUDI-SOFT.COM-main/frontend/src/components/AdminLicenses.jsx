                                ...
                                <tr key={lic._id} className="hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${
                                            lic.productType === 'POS_ONLINE' || lic.productType === 'POS' ? 'bg-blue-500/20 text-blue-400' : 
                                            lic.productType === 'HMS' ? 'bg-teal-500/20 text-teal-400' :
                                            'bg-purple-500/20 text-purple-400'
                                        }`}>
                                            {lic.productType?.replace('_', ' ')}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="text-slate-400">{lic.subscriptionType === 'FiveYear' ? '5-Year' : 'Monthly'}</span>
                                            {lic.isTrial && <span className="text-[9px] text-teal-400 font-black uppercase tracking-tighter mt-1">Free Trial</span>}
