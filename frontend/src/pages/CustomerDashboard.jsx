            ...
            <main className="max-w-7xl mx-auto px-6 py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <ProductCard 
                        ...
                        pwaLink="https://daryeel-hms-com.vercel.app"
                    />

                    {/* Detail Care Card */}
                    <ProductCard 
                        title="Detail Care Clinic PWA" 
                        description="Specialized enterprise clinic Progressive Web App with video telehealth and EHR tracking."
                        productType="DETAIL_CARE"
                        licenses={licenses.filter(l => l.productType === 'DETAIL_CARE')}
                        onDownload={() => handleDownload('DETAIL_CARE')}
                        pwaLink="http://localhost:3000"
                    />
                </div>
            </main>
        </div>
...
const ProductCard = ({ title, description, productType, licenses, onDownload, pwaLink }) => {
    ...
    return (
        ...
        >
            ...
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 shadow-lg ${productType === 'DETAIL_CARE' ? 'bg-emerald-600 shadow-emerald-500/20' : productType === 'HMS' ? 'bg-rose-600 shadow-rose-500/20' : 'bg-indigo-600 shadow-indigo-500/20'} text-white`}>
                <Package size={28} />
            </div>
            ...
            <div className="space-y-4 mb-8">
                {activeLicense ? (
                    <>
                        ...
                        {pwaLink ? (
                            <a 
                                href={productType === 'HMS' 
                                    ? `${pwaLink}/activate?key=${activeLicense.licenseKey}`
                                    : `${pwaLink}/login?key=${activeLicense.licenseKey}`}
                                target="_blank"
