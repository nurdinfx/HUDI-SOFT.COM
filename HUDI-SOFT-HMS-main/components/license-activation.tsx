...
import { Shield, Key, Loader2, CheckCircle2, AlertCircle, Zap, Smartphone } from 'lucide-react';

interface LicenseActivationProps {
  onSuccess: (key: string) => void;
}

// ── Static License Definitions (Client Side Fallback) ──────────────────────
...
export function LicenseActivation({ onSuccess }: LicenseActivationProps) {
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Check Static Fallback first (Important for Offline/Static APKs)
      ...
      // 2. Pattern Match for dynamic keys
      ...
      setError('Invalid license key. Please check your key or contact support.');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#020817] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden relative z-10"
      >
        <div className="p-8 sm:p-10">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 text-sm font-medium leading-relaxed">Please enter your digital license key to unlock the HMS Professional suite.</p>
          </div>

          <form onSubmit={handleActivate} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-1">License Key</label>
          ...
          <div className="mt-8 text-center">
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em]">
              Need a key? <a href="https://hudisoft.online/request-demo" className="text-emerald-500 underline underline-offset-4">Request Trial</a>
            </p>
          </div>
        </div>
      </motion.div>
