...
router.get('/', async (req, res) => {
    try {
        ...
        if (!row) {
            ...
            const hospitalName = licRow.rows[0]?.hospital_name || baseRow?.name || 'Hospital';

            await db.prepare(`
                INSERT INTO hospital_settings (name, tagline, address, phone, email, website, currency, tax_rate, logo, zaad, sahal, edahab, mycash, pharmacy_zaad, pharmacy_sahal, pharmacy_edahab, pharmacy_mycash, tenant_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(
...
router.put('/', async (req, res) => {
    ...
    try {
        ...
        if (!existing) {
            await db.prepare('INSERT INTO hospital_settings (name, tagline, address, phone, email, website, currency, tax_rate, logo, zaad, sahal, edahab, mycash, pharmacy_zaad, pharmacy_sahal, pharmacy_edahab, pharmacy_mycash, tenant_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
                .run(name || 'Hospital', tagline || '', address || '', phone || '', email || '', website || '', currency || 'USD', taxRate || 10, logo || null, zaad || '', sahal || '', edahab || '', mycash || '', pharmacy_zaad || '', pharmacy_sahal || '', pharmacy_edahab || '', pharmacy_mycash || '', tenantId);
        } else {
