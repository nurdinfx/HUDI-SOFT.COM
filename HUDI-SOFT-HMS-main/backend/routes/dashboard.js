...
router.get('/', async (req, res) => {
  ...
  const thisMonthPattern = today.slice(0, 7); // e.g. "2026-06"
    ...
    try {
        const [
            ...
            db.prepare("SELECT SUM(paid_amount) as total FROM invoices WHERE LEFT(date::text, 7) = ?").get(thisMonthPattern),
