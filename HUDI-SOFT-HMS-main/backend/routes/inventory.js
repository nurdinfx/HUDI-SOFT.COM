...
router.get('/', async (req, res) => {
    ...
    let q = 'SELECT * FROM inventory_items WHERE tenant_id = ?'; const p = [tenantId];
...
router.get('/:id', async (req, res) => {
    const tenantId = req.tenantId;
    try {
        const row = await db.prepare('SELECT * FROM inventory_items WHERE id = ? AND tenant_id = ?').get(req.params.id, tenantId);
...
router.post('/', async (req, res) => {
    ...
    const tenantId = req.tenantId;
    try {
        const maxItemData = await db.prepare("SELECT item_id FROM inventory_items WHERE item_id LIKE 'INV-%' AND tenant_id = ? ORDER BY item_id DESC LIMIT 1").get(tenantId);
        ...
        await db.prepare(`INSERT INTO inventory_items (id, item_id, name, category, description, quantity, unit, reorder_level, unit_cost, supplier, last_restocked, status, tenant_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await db.prepare('UPDATE inventory_items SET name=?, category=?, description=?, quantity=?, unit=?, reorder_level=?, unit_cost=?, supplier=?, last_restocked=?, status=? WHERE id=? AND tenant_id=?')
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const row = await db.prepare('SELECT * FROM inventory_items WHERE id = ? AND tenant_id = ?').get(req.params.id, tenantId);
        if (!row) return res.status(404).json({ error: 'Not found' });
        await db.prepare('DELETE FROM inventory_items WHERE id = ? AND tenant_id = ?').run(req.params.id, tenantId);
